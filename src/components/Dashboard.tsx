...
import {
  dailyTotals, todayTotals, avgPerDrink, avgIntervalMs, wastePercent, totals, recommend, formatDuration,
} from '../lib/stats'
...
export default function Dashboard({ feeds, now, onGoLog }: Props) {
  ...
  if (feeds.length === 0) {
    return (
      <div className="mx-auto flex max-w-md flex-col items-center gap-4 py-10 text-center">
        <img
          src="/art/empty-hero.webp"
          alt=""
          className="h-52 w-52 object-contain drop-shadow"
        />
